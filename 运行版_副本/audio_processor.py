    ...
    def has_sufficient_audio(self, client_id: str, threshold: int = 3) -> bool:
        ...
        try:
            if client_id in self.audio_buffers:
                ...
                has_sufficient = buffer_size >= threshold
                
                if has_sufficient:
                    logger.debug(f"✅ 客户端 {client_id} 音频数据充足: {buffer_size}/{threshold}")
                else:
                    logger.debug(f"⏳ 客户端 {client_id} 音频数据不足: {buffer_size}/{threshold}")
